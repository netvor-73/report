...
We start by labeling the axies $\textbf{x} = \{x_{1}, x_{2}\}$.
...
\Cref{equ:b_likelihood} is usually the cost function used for Bernoulli distributed labelled data. It is ofter refered to as \textbf{binary cross-entropy} or BCE for short.
...
\begin{figure}[!htpb]
  ...
  \caption{A visual representation of a feedforward network which approximates some function $f : \mathbb{R}^n \longrightarrow \mathbb{R}^m$ by
    ...
    directed weighted graph. Here $x = (x_1, \ldots, x_n)$}
\end{figure}
